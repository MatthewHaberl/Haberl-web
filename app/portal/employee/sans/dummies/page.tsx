...
/** "SANS for Dummies" — pick a chapter, read it in plain English. */
export default async function SansDummiesPage() {
  ...
  const top = ((data as SansClause[] | null) ?? []).filter(
    (c) => /^\d+$/.test(c.clause_ref) || isAnnexRef(c.clause_ref),
  )
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Every clause rewritten in human words. Read a whole chapter in minutes and know what the standard actually wants — then jump to the verbatim text when you need the letter of the law."
        icon={Lightbulb}
      />
