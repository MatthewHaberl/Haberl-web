...
for (const f of ['tables-1.json', 'tables-2.json', 'tables-3.json', 'tables-4.json', 'tables-8.json', 'tables-9.json']) {
...
// conductor-impedance.ts — table D.1 (R/X in Ω/km, for PSCC and Annex D calculations)
{
  const t = byId.get('D.1')!
  const rows = t.rows.map((r) =>
    `  { size: ${fmt(r.size_mm2)}, rAcCu: ${fmt(r.r_ac_cu)}, rAcAl: ${fmt(r.r_ac_al)}, xAcCu: ${fmt(r.x_ac_cu)}, xAcAl: ${fmt(r.x_ac_al)}, rDcCu: ${fmt(r.r_dc_cu)}, rDcAl: ${fmt(r.r_dc_al)} },`,
  ).join('\n')
  const src = `/**
...
`
  writeFileSync(join(OUT_DIR, 'conductor-impedance.ts'), src)
  console.log('wrote conductor-impedance.ts')
}

// spd-risk.ts — Annex Q tables Q.1.1/Q.1.2 (requirement bands) + Q.1.3 (town Ng) + I.1 (impulse categories)
{
  function bands(t: JsonTable): string {
    return t.rows.map((r) =>
      `  { ngMin: ${fmt(r.ng_min)}, ngMaxExclusive: ${fmt(r.ng_max_exclusive)}, byEnvironment: { ` +
      (['rural', 'suburban', 'urban'] as const).map((env) =>
        `${env}: { exceedsM: ${fmt(r[`${env}_service_line_exceeds_m`])}, spdType: ${fmt(r[`${env}_spd_type`])}, ratingKa: ${fmt(r[`${env}_spd_rating_ka`])} }`,
      ).join(', ') + ' } },',
    ).join('\n')
  }
  const q11 = byId.get('Q.1.1')!
  const q12 = byId.get('Q.1.2')!
  const q13 = byId.get('Q.1.3')!
  const i1 = byId.get('I.1')!
  const towns = q13.rows.map((r) => `  { town: ${fmt(r.town)}, ng: ${fmt(r.ng)} },`).join('\n')
  const impulse = i1.rows.map((r) =>
    `  { maxLineToNeutralV: ${fmt(r.voltage_line_to_neutral_max_v)}, catI: ${fmt(r.rated_impulse_voltage_cat_i_v)}, catII: ${fmt(r.rated_impulse_voltage_cat_ii_v)}, catIII: ${fmt(r.rated_impulse_voltage_cat_iii_v)}, catIV: ${fmt(r.rated_impulse_voltage_cat_iv_v)} },`,
  ).join('\n')
  const src = `/**
...
`
  writeFileSync(join(OUT_DIR, 'spd-risk.ts'), src)
  console.log('wrote spd-risk.ts')
}
