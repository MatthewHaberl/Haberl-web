...
/**
 * Amendment 3 (Ed 3.03) rules binding PV/battery work.
 *
 * Matthew's call (2026-08): enforce these NOW rather than from the effective
 * date — a job designed to Ed 3.2 today may be inspected against Amdt 3 by the
 * time its CoC is issued, and every rule is additive over Ed 3.2.
 *
 * machine_checkable is true only where the engine can actually prove the
 * verdict from the BOM. The rest are surfaced on every design as warnings the
 * installer must action; implemented_in records where that happens.
 */
const AMDT3_IMPL = 'lib/solar/compliance.ts amendment3Checks()'
const amdt3Rows: RuleRow[] = [
  { rule_key: 'AMDT3-MULTI-01', title: 'One source per multicore cable', summary: 'A multicore cable may not contain conductors from circuits supplied by different sources — e.g. inverter input and inverter output must not share a multicore. Also: one disconnecting device per multicore cable.', clause_refs: ['6.1.13', '6.9.1'], category: 'Cabling, Conduit & Glands', severity: 'blocker', machine_checkable: false, implemented_in: AMDT3_IMPL, source: 'amdt3' },
  { rule_key: 'AMDT3-NE-01', title: 'Island-mode N-E bond must live inside the converter', summary: 'For backup-capable inverters/generators, neutral and PE may only be connected while intentionally in island mode, and the reference must be internal to the unit or manufacturer-supplied control gear. An external relay/contactor arrangement is not accepted.', clause_refs: ['7.12.3'], category: 'AC & Distribution Board', severity: 'blocker', machine_checkable: true, implemented_in: AMDT3_IMPL, source: 'amdt3' },
  { rule_key: 'AMDT3-PSCC-01', title: 'Sum PSCC across paralleled sources', summary: 'Where more than one source can operate in parallel (grid + PV + battery), prospective short-circuit current must be summed. Battery PSCC comes from the BMS nameplate, datasheet, manufacturer or calculation.', clause_refs: ['8.4'], category: 'Compliance & Paperwork', severity: 'warning', machine_checkable: false, implemented_in: AMDT3_IMPL, source: 'amdt3' },
  { rule_key: 'AMDT3-INSUL-01', title: 'PV array DC insulation-resistance test', summary: 'PV array DC circuits get a dedicated insulation-resistance test per IEC 62446-1:2018, separate from the AC tests; minimum 1,0 MΩ.', clause_refs: ['8.6.8'], category: 'Compliance & Paperwork', severity: 'warning', machine_checkable: false, implemented_in: AMDT3_IMPL, source: 'amdt3' },
  { rule_key: 'AMDT3-STRUCT-01', title: 'PV mounting needs structural sign-off', summary: 'PV support structures and module mounting must comply with building regulations and SANS 60364-7-712 structural load requirements; mechanical soundness may need a competent-person opinion; roof penetrations must be weatherproof.', clause_refs: ['8.6.15'], category: 'Compliance & Paperwork', severity: 'warning', machine_checkable: false, implemented_in: AMDT3_IMPL, source: 'amdt3' },
  { rule_key: 'AMDT3-FIRE-01', title: 'PV fire-risk compliance', summary: 'Fire risks must comply with SANS 60364-7-712 plus national and local fire regulations applicable to the installation.', clause_refs: ['8.6.16'], category: 'Compliance & Paperwork', severity: 'warning', machine_checkable: false, implemented_in: AMDT3_IMPL, source: 'amdt3' },
  { rule_key: 'AMDT3-REPORT-01', title: 'Annex S test report for the PV/battery portion', summary: 'Where an installation includes a generating plant, an Annex S test report (converters, storage, PV modules, DC voltage, islanding settings, SLD, bonding continuity, ECC sizing, neutral loop impedance, PSCC, elevated N-E voltage) must be attached for the generating-plant portion, with a registered-person declaration.', clause_refs: ['8.7', 'S'], category: 'Compliance & Paperwork', severity: 'blocker', machine_checkable: true, implemented_in: AMDT3_IMPL, source: 'amdt3' },
]
